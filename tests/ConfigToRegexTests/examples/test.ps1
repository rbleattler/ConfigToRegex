...
if (-not $skipBuild) {
  ...
  Import-Module .\src\ConfigToRegex\bin\Release\net8.0\publish\ConfigToRegex.dll
} else {
  try {
    Import-Module .\src\ConfigToRegex\bin\Release\net8.0\publish\ConfigToRegex.dll
  } catch {
...
if ($iterative) {
  ...
  function New-ConfigToRegexObject {
    [CmdletBinding()]
    param(
      [string]$name,
      [string]$content
  ...
  $vars | ForEach-Object {
    ...
    Write-Verbose "---"
    Write-Host "`n"
    $obj = New-ConfigToRegexObject -name $PSItem.Name -content $PSItem.Value
    if ($null -ne $obj) {
...
# $pv = [ConfigToRegex.PatternValue]::new($patternValue)
# $q = [ConfigToRegex.Quantifier]::new($testQuantifier)
# $p = [ConfigToRegex.Pattern]::new($testPattern)
# # $p2 = [List[ConfigToRegex.Pattern]]::new($testPattern2)
# $gp = [ConfigToRegex.GroupPattern]::new($groupPattern)
# $gp2 = [ConfigToRegex.GroupPattern]::new($groupPattern2)
# $pp = [ConfigToRegex.PatternProperties]::new($patternProperties)
# $ap = [ConfigToRegex.AnchorPattern]::new($anchorPattern)
